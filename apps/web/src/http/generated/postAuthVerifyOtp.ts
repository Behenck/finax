...
export async function postAuthVerifyOtp({ data }: { data: PostAuthVerifyOtpMutationRequest }, config: Partial<RequestConfig<PostAuthVerifyOtpMutationRequest>> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<PostAuthVerifyOtpMutationResponse, ResponseErrorConfig<Error>, PostAuthVerifyOtpMutationRequest>({ method : "POST", url : getPostAuthVerifyOtpUrl().url.toString(), data : requestData, ... requestConfig })  
  return res.data
}