...
export async function postInvitesInviteidAccept({ inviteId }: { inviteId: PostInvitesInviteidAcceptPathParams["inviteId"] }, config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<PostInvitesInviteidAcceptMutationResponse, ResponseErrorConfig<Error>, unknown>({ method : "POST", url : `/invites/${inviteId}/accept`, baseURL : `http://localhost:3333`, ... requestConfig })  
  return res.data
}
