			...
			<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
				...
				<Card className="p-5 space-y-2">
					<div className="flex items-center justify-between">
						...
						<Badge variant="outline">{customer.status === "ACTIVE" ? "Ativo" : "Inativo"}</Badge>
					</div>
				...
				<Card className="p-5 space-y-2">
					...
					<p className="text-lg font-semibold">{delinquentSalesCount} venda(s)</p>
					<p className="text-sm text-muted-foreground">
			...
			<div className="grid gap-4 lg:grid-cols-2">
				<Card className="p-6 space-y-3">
					...
					<div className="space-y-2 text-sm">
						<p>
							<strong>Documento:</strong> {getDocumentTypeLabel(customer.documentType)} {customer.documentNumber}
						</p>
						<p>
				...
				<Card className="p-6 space-y-3">
					...
					<div className="space-y-2 text-sm">
						{customer.pf ? (
							<>
								<p>
									<strong>Data de nascimento:</strong> {formatDateTime(customer.pf.birthDate)}
								</p>
								<p>
									<strong>Profissão:</strong> {customer.pf.profession ?? "Não informado"}
								</p>
								<p>
									<strong>Naturalidade:</strong> {customer.pf.naturality ?? "Não informado"}
								</p>
							</>
						) : null}
						{customer.pj ? (
							<>
								<p>
									<strong>Razão social:</strong> {customer.pj.legalName ?? "Não informado"}
								</p>
								<p>
									<strong>Nome fantasia:</strong> {customer.pj.tradeName ?? "Não informado"}
								</p>
								<p>
									<strong>Fundação:</strong> {formatDateTime(customer.pj.foundationDate)}
								</p>
							</>
						) : null}
			...
			{canViewSales ? (
				<section className="space-y-3">
					<div className="space-y-1">
						...
						<p className="text-sm text-muted-foreground">
							As vendas com inadimplência aparecem destacadas para facilitar a identificação de possível risco de estorno.
						</p>
					</div>
					<CustomerSalesList sales={customer.sales} />
				</section>
			) : null}
		</main>
	);
