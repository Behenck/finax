...
export function DeletePartner({ partner }: DeletePartnerProps) {
	...
	const { mutateAsync: deletePartner, isPending } =
		useDeleteOrganizationsSlugPartnersPartnerid();
	...
	async function handleDeletePartner() {
		...
		try {
			...
			toast.success(
				`Parceiro ${getPartnerDisplayName(partner)} excluído com sucesso!`,
			);
			setOpen(false);
		} catch (error) {
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>{getPartnerDisplayName(partner)}</strong>? Esta ação não
							pode ser desfeita.
						</AlertDialogDescription>
					</AlertDialogHeader>
