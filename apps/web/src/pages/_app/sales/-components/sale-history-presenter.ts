...
function getSourceTypeLabel(value: unknown) {
  ...
  return (
    SALE_COMMISSION_SOURCE_TYPE_LABEL[value as SaleCommissionSourceType] ??
    value
  );
}
...
function getDirectionLabel(value: unknown) {
  ...
  return (
    SALE_COMMISSION_DIRECTION_LABEL[value as SaleCommissionDirection] ?? value
  );
}
