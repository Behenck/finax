...
export async function loadCustomersResponsible(
  organizationId: string,
  customers: Array<{ id: string; responsibleType?: string | null; responsibleId?: string | null }>,
) {
  ...
  const entries: Array<[string, CustomerResponsiblePayload | null]> = customers.map((customer) => {
      if (customer.responsibleType === "SELLER" && customer.responsibleId) {
