...
function parseSaleStatus(rawValue: unknown): SaleStatus | null {
  ...
  const statusByLabel = new Map<string, SaleStatus>([
    ...
    ["confirmada", SaleStatus.APPROVED],
    ["aprovada", SaleStatus.APPROVED],
    ["approved", SaleStatus.APPROVED],
...
function parseCommissionInstallment(rawValue: unknown) {
  ...
  const preserveNullExpectedPaymentDate =
    sanitizeTextValue(item.data_vencimento, { maxLength: 40 }) ===
    "0000-00-00";
  ...
  for (const partner of context.partners) {
    if (
      ...
      suggestions.push({
        type: "PARTNER",
        id: partner.id,
        label: partner.companyName || partner.name,
        email: partner.email,
      });
    }
  }
