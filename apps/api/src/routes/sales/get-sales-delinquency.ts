...
} from "./sale-delinquencies";
import { loadSalesResponsible } from "./sale-responsible";
import { SaleDelinquencyListItemSchema } from "./sale-schemas";

export async function getSalesDelinquency(app: FastifyInstance) {
  app
    .withTypeProvider<ZodTypeProvider>()
    .register(auth)
    .get(
      "/organizations/:slug/sales/delinquency",
      {
        schema: {
          tags: ["sales"],
          ...
          response: {
            200: z.object({
              sales: z.array(SaleDelinquencyListItemSchema),
            }),
          },
        },
      },
      async (request) => {
        ...
        const [delinquencySummaryBySaleId, openDelinquenciesBySaleId] =
          await Promise.all([
            loadSaleDelinquencySummaryBySaleIds(
              prisma,
              organization.id,
