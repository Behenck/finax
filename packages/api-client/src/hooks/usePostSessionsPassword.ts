...
export async function postSessionsPassword({ data }: { data: PostSessionsPasswordMutationRequest }, config: Partial<RequestConfig<PostSessionsPasswordMutationRequest>> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<PostSessionsPasswordMutationResponse, ResponseErrorConfig<PostSessionsPassword401>, PostSessionsPasswordMutationRequest>({ method : "POST", url : `/sessions/password`, data : requestData, ... requestConfig })  
  return res.data
}
